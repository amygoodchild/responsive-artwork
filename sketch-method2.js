// Normalised dimensions scaled mathematically
...
function createArt(){
  // Every dimension is based on a 1000px canvas and recalculated later
  artCircle = {
...
function scaleArt(){
  ...
  artCircle.pos.x = artCircle.pos.x * sz.f.x;
  artCircle.pos.y = artCircle.pos.y * sz.f.y;
  artCircle.width = artCircle.width * sz.f.x;
  artCircle.strokeWeight = artCircle.strokeWeight * sz.f.x;
}
...
function handleResize(){
  ...
  // Resize the art
  sz.f.x = sz.w / oldWidth;
  sz.f.y = sz.h / oldHeight;
  scaleArt();
